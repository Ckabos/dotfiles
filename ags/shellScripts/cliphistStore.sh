...
# --- 1. Limpieza estricta de procesos para evitar duplicados al recargar OkPanel ---
killall -q wl-paste
# Esperar a que los procesos mueran completamente
while pgrep -u $UID -x wl-paste >/dev/null; do sleep 0.1; done
...
# --- 2. Iniciar demonios limpios en segundo plano ---
# Ejecutar el filtro de seguridad para texto
wl-paste --type text --watch bash -c cliphist_filter &

# Ejecutar el guardado estándar para imágenes (el filtro bash es solo para texto)
