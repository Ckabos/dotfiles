...
function updateWeather() {
    ...
    fetchJson(url)
        ...
        .catch((error) => {
            console.warn(error); // Cambiado a console.error nativo
        })
        .finally(() => {
...
export function getWeatherIcon(code: number | null, isDay: boolean = true): string {
    ...
    switch (code) {
        case 0: return isDay ? nf.sun : nf.moon;
        case 1:
        case 2: return isDay ? nf.partlyCloudyDay : nf.partlyCloudyNight;
        case 3: return nf.overcast;
        case 45:
        case 48: return nf.fog;
        case 51:
        case 53:
        case 55: return nf.drizzle;
        case 56:
        case 57: return nf.sleet;
        case 61:
        case 63:
        case 65: return nf.rain;
        case 66:
        case 67: return nf.sleet;
        case 71:
        case 73:
        case 75:
        case 77: return nf.snow;
        case 80:
        case 81:
        case 82: return nf.rain;
        case 85:
        case 86: return nf.snow;
        case 95:
        case 96:
        case 99: return nf.thunderstorm;
        default: return nf.unknown;
    }
}
...
function setupUpdateInterval() {
    if (updateIntervalBinding !== null) return;
    ...
    const unsub = updateIntervalBinding.subscribe(() => {
        ...
        if (network.connectivity === AstalNetwork.Connectivity.FULL) {
            updateInterval = interval(WEATHER_UPDATE_INTERVAL, () => updateWeather())
        }
    })
...
export default function() {
    ...
    const hourlyWeather = createComputed([weather], (w) => {
        const now = new Date()
        return w.hourly.filter((h) => h.time >= now).slice(0, 4)
    })

    const dailyWeather = createComputed([weather], (w) => {
        return w.daily.slice(0, 4)
    })
    ...
    return <box
        ...
        <label
            ...
            label={weather.as((w) => {
                ...
                if (code == null || isDay == null || temp == null) return "N/A";
        ...
        <box
            ...
            <box orientation={Gtk.Orientation.VERTICAL}>
                <label
                    ...
                    label={weather.as((w) => w?.current?.humidity != null ? `  ${w.current.humidity}%` : "N/A")}
                />
                <label cssClasses={["labelSmall"]} label="Humedad"/>
            </box>

            <box orientation={Gtk.Orientation.VERTICAL}>
                <label
                    cssClasses={["labelMedium"]}
                    label={weather.as((w) => w?.current?.uvIndex != null ? `󱩅 ${w.current.uvIndex}` : "N/A")}
                />
                <label cssClasses={["labelSmall"]} label="Índice UV"/>
            </box>
            <box orientation={Gtk.Orientation.VERTICAL}>
                <label
                    cssClasses={["labelMedium"]}
                ...
                />
                <label cssClasses={["labelSmall"]} label="Viento"/>
            </box>
        </box>
        <label
            marginTop={20}
                    ...
                    return <box orientation={Gtk.Orientation.VERTICAL}>
                        <label
                            ...
                            label={variableConfig.clockFormat24h.asAccessor().as((format24h) => {
                                return hourly.time.toLocaleTimeString('es-MX', { hour: 'numeric', hour12: !format24h })
                            })}/>
                        <label
                        ...
                        <label
                            cssClasses={["labelSmall"]}
                            label={hourly.temperature != null ? `${hourly.temperature}${variableConfig.weather.temperatureUnit.get() === TemperatureUnits.F ? "F" : "C"}` : "N/A"}
                        />
                        <label
                            cssClasses={["labelSmall"]}
                            label={hourly.uvIndex != null ? `󱩅 ${hourly.uvIndex}` : "N/A"}
                        />
                    </box>
                }}
            </For>
            ...
            <For each={dailyWeather}>
                {(daily) => {
                    return <box orientation={Gtk.Orientation.VERTICAL}>
                        <label
                            cssClasses={["labelSmall"]}
                            label={daily.time.toLocaleDateString('es-MX', { weekday: 'short', timeZone: 'UTC' })}/>
                        <label
                            cssClasses={["labelLarge"]}
                            label={getWeatherIcon(daily.weatherCode, true)}/>
                        <label
                            cssClasses={["labelSmall"]}
                            label={daily.maxTemp != null ? `${daily.maxTemp}${variableConfig.weather.temperatureUnit.get() === TemperatureUnits.F ? "F" : "C"}` : "N/A"}
                        />
                        <label
                            cssClasses={["labelSmall"]}
                            label={daily.minTemp != null ? `${daily.minTemp}${variableConfig.weather.temperatureUnit.get() === TemperatureUnits.F ? "F" : "C"}` : "N/A"}
                        />
                    </box>
                }}
            </For>
