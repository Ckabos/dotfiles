...
import SwapWorkspaceButton from "./SwapWorkspaceButton";
import NetTraffic from "./NetTraffic";
...
export function addWidgets(widgets: BarWidget[], bar: Bar) {
    ...
    return widgets.map((widget) => {
        const wConfig = variableConfig.barWidgets[widget];

        return <box
            marginTop={wConfig?.marginTop?.asAccessor() || 0}
            marginBottom={wConfig?.marginBottom?.asAccessor() || 0}
            marginStart={wConfig?.marginStart?.asAccessor() || 0}
            marginEnd={wConfig?.marginEnd?.asAccessor() || 0}>
...
function getWidget(widget: BarWidget, isVertical: boolean, bar: Bar) {
    switch (widget) {
        ...
        case BarWidget.IP_INDICATOR:
            return <IpWidget bar={bar}/>
        case BarWidget.PORT_MONITOR:
            return <PortMonitor />
        case BarWidget.TARGET_TRACKER:
            return <TargetTracker />
        case BarWidget.SWAP_WORKSPACE:
            return <SwapWorkspaceButton />
